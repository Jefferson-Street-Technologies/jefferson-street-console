...
5. **Implement the Textual `Screen`** so it mutates `session` only. When adding a resource, also call `app.remember_label(id, label)` so host UI can show names. Rely on host `s` / `e` for shared session/export UI; only declare step-local keys on `bindings` and implement matching `action_*` methods.
...
| Inside the TUI | `s` session · `e` export · `n`/`p` navigate · `?` step keys · `q` quit |
