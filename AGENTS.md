...
1. **Session** — portable analytical intent (`metric` / `entity` / `series` + `head`/`tail`/`as_of`/`sort_by`/`taxonomy`). Mirrors `JSTDataClient.query` parameters. No observations; can execute, export CSV, or render Python/CLI snippets. **Source of truth** for what is staged. Deep history uses `GET /series/{id}/observations`, not `/query` date windows.
...
4. **Import the module** from `jstdata/workflows/__init__.py` so it registers (same pattern as `console` / `discover` / `rank`).
